...
router.get('/TorreMIN', async function(req, res){
	sql.close();
    ...
    const request = new sql.Request();
		...
		And dl2.SourceID between `+req.query.SIDInicio+` And `+req.query.SIDFinal+` AND Value is not null Order by Value ASC)`, (error, minimo) =>{
			if (error) console.log(error);
			console.log('Consumo minimo');
